...
//appel de la fonction TabOAlph
$Tab = TabOAlph($langage);
//parcours chaque le tableau pour afficher son contenu
foreach($Tab as $element) {
  echo $element;
}
//déclaration de la fonction TabOAlph() permettant de tirer par ordre alphabétique
function TabOAlph($langage) {
  //tri du tableau
  sort($langage);
  return $langage;
}
//déclaration de la fonction chain() qui permet de renvoyer le contenu d'un tableau concaténé
function chain($langage) {
  $chf = implode(' ', $langage); /*inverse() de implode c'est explode()*/
  return($chf);
}
echo '<br />';
$chf = chain($langage);
